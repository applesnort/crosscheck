...
// How much of a file a lens needs to see. `hunks` sends only changed spans plus
// context, which is far cheaper on a large file and wrong for any lens that
// follows data or judges structure — so it is the lens author's call, declared
// per lens rather than imposed by a global flag.
export const LENS_SCOPES = ['file', 'hunks'];

// Reasoning depth, passed to the runner as CROSSCHECK_EFFORT. crosscheck cannot
// know a provider's flag for this, so it publishes the intent and lets the
// runner translate it.
export const LENS_EFFORTS = ['low', 'medium', 'high'];

// An invariant is the unit that lets a lens lower its own capability floor. It
// pairs a rule for what to LOOK AT with a rule for what an observation MEANS,
// and both halves are required because they fail differently. Measured on the
// calibration fixture: an observation rule alone produced a model that
// correctly enumerated the offending function and then recorded it as
// acceptable, having been told what to find and not what it signified.
//
// Parsed from the lens body rather than its frontmatter: these are paragraphs,
// and the frontmatter parser reads scalars and inline lists on purpose.
const INVARIANT_HEAD = /^###\s+(\S+)\s*[—–-]\s*(.+?)\s*$/;
const INVARIANT_FIELD = /^\*\*(observe|verdict|canary):\*\*\s*([\s\S]*)$/;

export function parseInvariants(body) {
  ...
  const flushField = () => {
    if (current && field) {
      current[field.name] = field.lines.join('\n').trim();
    }
    field = null;
  };
  ...
  for (const line of lines) {
    if (/^##\s+/.test(line)) {
      // Any other level-2 heading closes the section, so prose that follows is
      // not silently absorbed into the last invariant's verdict.
      flushInvariant();
      inSection = /^##\s+Invariants\s*$/i.test(line);
      continue;
    }
    if (!inSection) {
      continue;
    }
    const head = INVARIANT_HEAD.exec(line);
    if (head) {
      flushInvariant();
    ...
    const fieldStart = INVARIANT_FIELD.exec(line);
    if (fieldStart) {
      flushField();
      field = { name: fieldStart[1], lines: [fieldStart[2]] };
      continue;
    }
    if (field) {
...
export function validateLens(meta) {
  ...
  // Both are optional, and a value that is present but unrecognised is a typo.
  // Defaulting it quietly would bill the user for a setting they believe is on.
  if (meta.scope != null && !LENS_SCOPES.includes(meta.scope)) {
    problems.push(
      `\`scope\` must be one of ${LENS_SCOPES.join(', ')} (got "${meta.scope}")`);
  }
  if (meta.effort != null && !LENS_EFFORTS.includes(meta.effort)) {
    problems.push(
      `\`effort\` must be one of ${LENS_EFFORTS.join(', ')} (got "${meta.effort}")`);
  }
  for (const inv of meta.invariants ?? []) {
    const id = inv?.id ?? '<unnamed>';
    if (!inv?.observe) {
      problems.push(
        `invariant ${id} has no \`observe:\` rule — an invariant with nothing ` +
        'to look at cannot be checked against the file');
    }
    if (!inv?.verdict) {
      problems.push(
        `invariant ${id} has no \`verdict:\` rule — without one a lens reports ` +
        'what it saw and leaves whether it matters to a model that will ' +
        'under-call it');
    }
  }
