...
export function buildLensPrompt(lens, files, options = {}) {
  ...
  // An abstention that says only "nothing here" is compatible with a clean
  // file, a lens that could not recognise the defect, and a lens whose
  // questions exceed the runner. It forbids nothing, so it cannot be wrong, so
  // it is not a claim. Coverage lines make it one: they name what was examined,
  // and a reader can check them against the file and find them false.
  const coverage = invariants.length
    ? 'Before that line, emit one COVERAGE line per invariant:\n\n' +
      '  COVERAGE: <invariant id> — the sites you examined for it\n\n' +
      'Emit these whether or not you found anything. An invariant you could ' +
      'not check states that, and why.'
    : 'Before that line, emit one COVERAGE line naming what you examined:\n\n' +
