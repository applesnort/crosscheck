...
export function mergeFindings(reports, options = {}) {
  ...
  // record.
  const unsupported = [];
  // A control is a known violation placed in the lens's scope. Catching it
  // shows the runner can perform that check; missing it shows it cannot, and
  // that outranks the lens's own account of what it examined. Coverage is a
  // claim about looking. A failed control is a demonstration of not seeing.
  ...
  for (const report of reports ?? []) {
    ...
    const expected = report.controlsExpected ?? [];
    const caught = report.controls ?? [];
    const missed = expected.filter(id => !caught.includes(id));
    if (missed.length) {
      undemonstrated.push({ lens: report.lens ?? '<unknown>', missed });
    }
    if (report.findings.length === 0) {
      silent.push(report.lens ?? '<unknown>');
      if (!(report.coverage ?? []).length || missed.length) {
  ...
  return { findings, incomplete, unparsed, silent, unsupported, undemonstrated };
}
...
export function panelVerdict(counts, participation = null) {
  ...
  const undemonstrated = participation?.undemonstrated ?? [];
  const total = participation?.total ?? 0;
  // A lens that missed a control did not merely stay quiet; it showed it cannot
  // run the check it was given. No amount of quiet elsewhere makes that a pass.
  if (undemonstrated.length) {
    return 'No signal — ' + undemonstrated.map(u =>
      `${u.lens} could not demonstrate ${u.missed.join(', ')}`).join('; ') +
      '. A lens that misses a known violation placed in its own scope has not ' +
      'been shown capable of the check, so its silence carries nothing';
  }
