...
export function mergeFindings(reports, options = {}) {
  ...
  return { findings, incomplete, unparsed, silent };
}
...
export function panelVerdict(counts, participation = null) {
  ...
  if (silent.length >= total) {
    return 'No signal — every lens reported nothing. This is not a pass; ' +
      'a panel that found nothing anywhere has not been shown to be looking';
  }
  return `Ship, with ${silent.length} of ${total} lens(es) reporting nothing ` +
    `(${silent.join(', ')}) — their coverage is unconfirmed, not clean`;
}
