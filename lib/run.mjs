...
export async function runPanel({
  ...
  const reports = await mapLimit(jobs, concurrency, async job => {
    ...
    if (key) {
      ...
      if (cached != null) {
        const { findings, unparsed, coverage } = parseLensOutput(cached);
        onCacheHit?.(job.lens);
        onLensDone?.(job.lens, {
          ok: true, findings: findings.length, cached: true
        });
        return { lens: job.lens, findings, unparsed, coverage, output: cached };
      }
    }
    let result;
    try {
      ...
      return {
        lens: job.lens, findings: null, unparsed: [], coverage: [], output: null
      };
    }
    ...
    const { findings, unparsed, coverage } = parseLensOutput(stdout);
    if (key) {
    ...
    return { lens: job.lens, findings, unparsed, coverage, output: stdout };
  });
