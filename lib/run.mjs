...
import { planTokenBudget, formatEstimate } from './budget.mjs';
...
export function promptsFor(roster, options = {}) {
  return roster.map(lens => ({
    lens: lens.name,
    files: lens.files,
    controlsExpected: (lens.invariants ?? [])
      .filter(inv => inv.canary).map(inv => inv.id),
...
// Two-stage triage: a cheap pass over the whole target, then the real panel over
// only the files the cheap pass found something in. Most code is clean, so most
// files never reach the expensive runner.
//
// A lens that did not complete has `findings: null`, which is not the same as
// finding nothing. Reading it as clean would drop its files from the pass that
// would actually have caught the defect — a failure that looks like a saving.
export function narrowRosterToFindings(roster, reports) {
  const incomplete = (reports ?? []).some(r => r.findings == null);
  if (incomplete) {
    return roster ?? [];
  }
  const hot = new Set();
  ...
  return (roster ?? [])
    .map(lens => ({ ...lens, files: lens.files.filter(f => hot.has(f)) }))
    .filter(lens => lens.files.length > 0);
}
...
export async function runPanel({
  ...
  const { run: afterDispatchCap, dropped } = planBudget(allJobs, maxDispatches);

  // The estimate is reported whether or not a budget is set — a user asked to
  // justify the spend needs the number even on an uncapped run.
  const plan = planTokenBudget(afterDispatchCap, tokenBudget);
  onEstimate?.(formatEstimate(plan.estimated, plan.budget), plan);
  const jobs = plan.run;
  ...
  const reports = await mapLimit(jobs, concurrency, async job => {
    ...
    if (key) {
      ...
      if (cached != null) {
        const {
          findings, unparsed, coverage, controls
        } = parseLensOutput(cached);
        onCacheHit?.(job.lens);
        onLensDone?.(job.lens, {
          ok: true, findings: findings.length, cached: true
        });
        return {
          lens: job.lens, findings, unparsed, coverage, controls,
          controlsExpected: job.controlsExpected ?? [], output: cached
        };
      }
    }
    let result;
    try {
      ...
      return {
        lens: job.lens, findings: null, unparsed: [], coverage: [],
        controls: [], controlsExpected: [], output: null
      };
    }
    ...
    const { findings, unparsed, coverage, controls } = parseLensOutput(stdout);
    if (key) {
    ...
    return {
      lens: job.lens, findings, unparsed, coverage, controls,
      controlsExpected: job.controlsExpected ?? [], output: stdout
    };
  });
