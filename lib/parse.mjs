...
export function parseLensOutput(text) {
  ...
  return {
    findings, unparsed, coverage,
    noFindings: declined && findings.length === 0
  };
}
...
export function parseReports(reports) {
  return (reports ?? []).map(r => {
    if (!r || r.output == null) {
      return {
        lens: r?.lens ?? '<unknown>', findings: null, unparsed: [], coverage: []
      };
    }
    const { findings, unparsed, coverage } = parseLensOutput(r.output);
    return { lens: r.lens, findings, unparsed, coverage };
  });
}
