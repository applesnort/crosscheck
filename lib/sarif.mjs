...
function resultFor(finding) {
  ...
  return {
    ...
    partialFingerprints: { auditPanelFindingV1: fingerprint(finding) },
    properties: {
...
export function toSarif(merged, options = {}) {
  const {
    toolVersion = '0.1.0',
    informationUri = 'https://github.com/applesnort/audit-panel',
    lensMeta = {},
    refuted = []
  } = options;
  ...
  for (const lens of incomplete) {
    notifications.push({
      ...
      descriptor: { id: 'auditPanel/lensIncomplete' },
      properties: { lens }
    });
  }
  for (const { lens, line } of unparsed) {
    notifications.push({
      ...
      descriptor: { id: 'auditPanel/unparsedOutput' },
      properties: { lens }
    });
  }
  for (const finding of refuted) {
    notifications.push({
      ...
      descriptor: { id: 'auditPanel/refuted' },
